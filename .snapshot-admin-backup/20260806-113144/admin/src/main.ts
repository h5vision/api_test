...
type SnapshotAdminResponse = {
  repositories: SnapshotAdminRepository[];
  snapshots: SnapshotRecord[];
          ...
          <article class="panel rounded-2xl p-4 md:col-span-2 xl:col-span-3">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <p class="mt-1 text-xs leading-5 text-white/35">관리자에게 등록된 GitHub Snapshot Repository와 저장된 Snapshot 목록을 확인합니다.</p>
              </div>
              <span id="snapshot-admin-status" class="shrink-0 font-mono text-[10px] text-white/32">조회 중</span>
            </div>
            <div id="snapshot-admin-list" class="mt-3 space-y-2" aria-live="polite">
              <p class="text-[10px] text-white/30">Snapshot 저장 이력을 조회하고 있습니다.</p>
            </div>
          </article>
...
function snapshotAdminMarkup(data: SnapshotAdminResponse): string {
  if (!data.repositories.length && !data.snapshots.length) {
    return '<p class="text-[10px] text-white/30">저장된 Snapshot이 없습니다.</p>';
  }
  return data.repositories.map((repository) => {
    const snapshotsMarkup = repository.snapshots.length
      ? repository.snapshots.map((snapshot) => `
        <div class="rounded-lg border border-white/7 bg-black/10 px-2.5 py-2">
          <div class="flex items-start justify-between gap-2">
            <div class="min-w-0">
              <p class="truncate text-[10px] font-semibold text-white/70">${escapeHtml(snapshot.snapshot_id)}</p>
              <p class="mt-0.5 truncate font-mono text-[8px] text-white/24">${escapeHtml(snapshot.commit_sha.slice(0, 12))} · ${escapeHtml(snapshot.tree_sha.slice(0, 12))}</p>
            </div>
            <span class="shrink-0 text-[9px] font-semibold text-mint-300">${escapeHtml(snapshot.snapshot_type.toUpperCase())}</span>
          </div>
          <div class="mt-1.5 flex items-center justify-between gap-2 text-[9px] text-white/35">
            <span>${escapeHtml(formatClientDateTime(snapshot.created_at))}</span>
            <span class="truncate">${escapeHtml(snapshot.verified_by || "github")}</span>
          </div>
        </div>
      `).join("")
      : '<p class="text-[9px] text-white/30">이 Repository에는 Snapshot이 없습니다.</p>';
    return `
      <article class="rounded-xl border border-white/7 bg-white/2 p-3">
        <div class="flex items-start justify-between gap-2">
          <div class="min-w-0">
            <p class="truncate text-[11px] font-semibold text-white/75">${escapeHtml(repository.repository_full_name || repository.repository_id)}</p>
            <p class="mt-0.5 break-all font-mono text-[9px] text-white/30">${escapeHtml(repository.repository_id)} · ${escapeHtml(repository.default_branch || "branch 없음")}</p>
          </div>
          <span class="shrink-0 text-[9px] font-semibold text-amber-300">${repository.snapshot_count}개</span>
        </div>
        <div class="mt-2 space-y-1.5">${snapshotsMarkup}</div>
      </article>
    `;
  }).join("") || '<p class="text-[10px] text-white/30">저장된 Snapshot이 없습니다.</p>';
}

async function loadSnapshotAdminData(): Promise<void> {
  const list = document.getElementById("snapshot-admin-list");
  const status = document.getElementById("snapshot-admin-status");
  if (!list || !status) return;
  try {
    const response = await fetch(`${adminApiBaseUrl}/snapshots`, {
      headers: { Accept: "application/json" },
      cache: "no-store",
    });
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    const value = (await response.json()) as SnapshotAdminResponse;
    list.innerHTML = snapshotAdminMarkup(value);
    status.textContent = value.error
      ? `조회 경고 · ${value.error}`
      : `${value.total_repositories}개 Repository · ${value.total_snapshots}개 Snapshot`;
    status.className = `shrink-0 font-mono text-[10px] ${value.error ? "text-amber-300" : "text-white/32"}`;
  } catch (error) {
    const message = error instanceof Error ? error.message : "알 수 없는 오류";
    list.innerHTML = `<p class="text-[10px] text-danger-300">Snapshot 조회 실패: ${escapeHtml(message)}</p>`;
    status.textContent = "조회 실패";
    status.className = "shrink-0 font-mono text-[10px] text-danger-300";
  }
}
