...
def _load_router_module(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules,"backend.api",api)
    monkeypatch.setitem(sys.modules,"backend.api.v1",v1)
...
def test_repository_router_preserves_public_routes(monkeypatch):
    module,schemas=_load_router_module(monkeypatch)
